...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
  ...
  if (frame_) {
    frame_latch_ = std::shared_lock<std::shared_mutex>(frame_->rwlatch_);
    frame_->pin_count_++;
    if (replacer_) {
...
ReadPageGuard::ReadPageGuard(ReadPageGuard &&that) noexcept 
      : page_id_(that.page_id_),
        ...
        frame_latch_(std::move(that.frame_latch_)) {
          ...
          that.frame_latch_ = std::shared_lock<std::shared_mutex>();
        }
...
auto ReadPageGuard::operator=(ReadPageGuard &&that) noexcept -> ReadPageGuard & {
    if (this != &that) { // 避免自赋值
        ...
        if (frame_) {
            frame_->pin_count_--;
            if (replacer_) replacer_->SetEvictable(frame_->frame_id_, frame_->pin_count_ == 0);
        }
...
void ReadPageGuard::Flush() {
  ...
  auto data = const_cast<char *>(GetData());
  DiskRequest r{true, data, GetPageId(), std::promise<bool>{}};
...
void ReadPageGuard::Drop() {
  ...
  is_valid_ = false;
  frame_latch_ = std::shared_lock<std::shared_mutex>();
}
...
WritePageGuard::WritePageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      disk_scheduler_(std::move(disk_scheduler)) {
  if (frame_) {
    frame_latch_ = std::unique_lock<std::shared_mutex>(frame_->rwlatch_);
    frame_->pin_count_++;
    if (replacer_) {
      replacer_->SetEvictable(frame_->frame_id_, false);
    }
...
WritePageGuard::WritePageGuard(WritePageGuard &&that) noexcept       
      : page_id_(that.page_id_),
        ...
        frame_latch_(std::move(that.frame_latch_)) {
          ...
          that.disk_scheduler_ = nullptr;
          that.is_valid_ = false;
          that.frame_latch_ = std::unique_lock<std::shared_mutex>();
        }
...
void WritePageGuard::Drop() {
  ...
  disk_scheduler_ = nullptr;
  is_valid_ = false;
  frame_latch_ = std::unique_lock<std::shared_mutex>();
}
